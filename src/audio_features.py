...
Per PDF Section 3 (Preprocessing pipeline):
...
def load_audio(path: str | Path, sample_rate: int = TARGET_SR) -> tuple[np.ndarray, int]:
    """Load mono audio resampled to `sample_rate` (PDF: 22,050 Hz)."""
...
def log_mel_spectrogram(
    ...
    """Log-mel spectrogram with `n_mels` bins (PDF: 128). Shape (n_mels, time)."""
...
def chroma_features(
    ...
    """Chroma features with `n_chroma` bins (PDF: 12). Shape (n_chroma, time)."""
...
def segment_fixed(
    ...
    """
    Split feature matrix (n_features, time) into fixed-duration windows (PDF: 5–10 s).
...
def segment_beat_synchronous(
    ...
    """Beat-synchronous segmentation via librosa (PDF §3)."""
    ...
    # Each column is one beat-synchronous frame → treat consecutive pairs as segments
...
def build_fma_splits(
    ...
    """
    Official FMA splits (PDF §3): training / validation / test from tracks.csv.
...
def process_deam_audio(cfg: dict[str, Any], limit: int | None = None) -> None:
    ...
    print(f"[done] saved={ok} skipped={skipped} -> {processed}")
    print(
        "[note] DEAM train/val JSON needs DEAM annotations "
        "(not present under data/raw/deam yet)."
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="PDF §3 audio preprocessing + FMA splits")
